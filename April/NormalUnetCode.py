...
BATCH_SIZE = 2
EPOCHS = 500                  # Number of training epochs (adjust as needed)
LEARNING_RATE = 1e-4
...
RAW_DATA_DIR = r"C:\Users\ISU\cv_dataset\LocalDatasetCropped"
VAL_DIR = os.path.join(RAW_DATA_DIR, "val")
TEST_DIR = os.path.join(RAW_DATA_DIR, "test")
# Preprocessed (cached) training data directory
CACHED_TRAIN_DIR = r"C:\Users\ISU\cv_dataset\LocalDatasetCropped\ProcessedData\Train"
# Output directory for predictions
OUTPUT_MASK_PATH = os.path.join(TEST_DIR, "predictionsCUDATest16thApril")
...
class CachedMedicalDataset(Dataset):
    def _init_(self, processed_dir):
        # Assume for each image, there is a corresponding mask file.
...
class FocalTverskyLoss(nn.Module):
    def _init_(self, alpha=0.7, beta=0.3, gamma=0.75, smooth=1e-6):
